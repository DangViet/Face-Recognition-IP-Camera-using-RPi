...
def CropFace(image, eye_left=(0,0), eye_right=(0,0), offset_pct=(0.2,0.2), dest_sz = (70,70)):
    ...
    image.save("./temp/result/rotate.png")
    # crop the rotated image
    crop_xy = (eye_left[0] - scale*offset_h, eye_left[1] - scale*offset_v)
    crop_size = (dest_sz[0]*scale, dest_sz[1]*scale)
    image = image.crop((int(crop_xy[0]), int(crop_xy[1]), int(crop_xy[0]+crop_size[0]), int(crop_xy[1]+crop_size[1])))

    image.save("./temp/result/crop.png")
    # resize it
    image = image.resize(dest_sz, Image.ANTIALIAS)

    image.save("./temp/result/resize.png")
    return image
    ...
    for folder in next(os.walk(path))[1]:
        for file in os.listdir(path+'/'+folder):
            ...
            if len(eyes) == 2:
                ...
                if ex1 > ex2:
                    leftEye, rightEye = rightEye, leftEye
                cv2.line(array, leftEye, rightEye,(0,255,255),1)
                image = Image.fromarray(array)
                image.save("./temp/result/original.png")

                image = CropFace(image, eye_left=leftEye,eye_right=rightEye)
                break
                #cv2.imwrite('./temp/result/'+file,image, [cv2.IMWRITE_PNG_COMPRESSION, 0] )  
